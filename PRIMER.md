...
- **Long-running children must not inherit `postStart.sh`'s lock (v0.14.7+).** `postStart.sh` holds `flock -n 9`; `start_listener`/`start_audio_daemon` launch with `9<&-`. A child that inherits fd 9 holds the lock for as long as it runs, and every later `postStart.sh` (including "Restart FPPD", which runs only the start hooks) silently exits without restarting anything. Anything new started from `postStart.sh` needs the same `9<&-`.
