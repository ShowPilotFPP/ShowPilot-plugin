...
- The cooled item has already been started by FPP when the jump lands, so it is cut after milliseconds rather than never starting. A true zero-start skip needs an FPP change (e.g. letting `query_next` return a new next position); worth proposing upstream.
